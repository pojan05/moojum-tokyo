  ...
  return (
    <>
      ...
      <div className="min-h-screen bg-[#f8f9fa] font-sans text-slate-800 pb-32 select-none relative">
        ...
        <main className="max-w-md mx-auto p-4">
          ...
          {step === 4 && (
            <div className="animate-in zoom-in-95 duration-500 flex flex-col items-center mt-6">
              ...
              >
                <div className="absolute top-0 left-1/2 -translate-x-1/2 -translate-y-1/2 w-16 h-16 bg-emerald-100 rounded-full flex items-center justify-center shadow-inner">
                  <CheckCircle className="text-emerald-500 w-8 h-8" />
                </div>
                
                <div className="text-center mt-6 mb-6">
                  <h2 className="text-2xl font-black text-slate-800 tracking-tight">ส่งออเดอร์สำเร็จ!</h2>
                  <p className="text-slate-500 text-sm mt-1">หมูจุ่มโตเกียว Delivery</p>
                </div>

                <div className="border-t border-dashed border-slate-300 py-4 space-y-2 mb-4 text-left">
                  <p className="text-sm text-slate-600"><span className="font-bold text-slate-800">ผู้รับ:</span> {customerInfo.name}</p>
                  <p className="text-sm text-slate-600"><span className="font-bold text-slate-800">เบอร์โทร:</span> {customerInfo.phone}</p>
                  <p className="text-sm text-slate-600"><span className="font-bold text-slate-800">วิธีชำระเงิน:</span> {paymentMethod === 'cash' ? 'เงินสดปลายทาง' : 'โอนเงิน (พร้อมเพย์)'}</p>
                </div>
                    ...
                    return (
                      <div key={id} className="flex justify-between text-sm items-center">
                        <span className="text-slate-600 font-medium">
                          <span className="font-bold text-orange-600 mr-2">{qty}x</span> 
                          {item ? item.name : id}
                        </span>
                        <span className="font-bold text-slate-800">฿{item ? item.price * qty : 0}</span>
                      </div>
                    );
                  })}
                  <div className="border-t border-dashed border-slate-200 pt-3 mt-3 space-y-2">
                    <div className="flex justify-between text-sm items-center">
                      ...
                      <span className="font-bold text-slate-800">{isFreeDelivery ? 'ส่งฟรี' : `฿${activeDeliveryFee}`}</span>
                    </div>
                    {appliedDiscount && activeDeliveryFee > 0 && (
                      <div className="flex justify-between text-sm items-center text-emerald-600">
                        <span className="font-bold">ส่วนลด ({appliedDiscount.code})</span>
                ...
                <div className="flex justify-between items-end pt-4 border-t-2 border-slate-800">
                  <span className="font-black text-slate-800 text-lg">ยอดรวมทั้งสิ้น</span>
                  <span className="text-2xl font-black text-orange-600 tracking-tight">฿{finalTotalPrice}</span>
                </div>
              ...
              <button onClick={() => window.location.reload()} className="mt-8 bg-slate-200 hover:bg-slate-300 text-slate-700 px-6 py-3 rounded-2xl font-bold transition-colors w-full max-w-[200px]">
                สั่งออเดอร์ใหม่
              </button>
            </div>
          )}
        ...
        {/* Floating Bottom Bar (แสดงเฉพาะ Step 1 ถึง 3) */}
        {step === 1 && totalItems > 0 && (
        ...
        <div className="fixed bottom-6 left-4 right-4 z-40 animate-in slide-in-from-bottom-10 fade-in max-w-md mx-auto">
          {step === 2 ? (
            <button onClick={handleProceedToPayment} className="w-full bg-slate-900 text-white rounded-3xl py-4.5 font-black shadow-2xl flex justify-center items-center gap-2 hover:bg-slate-800 active:scale-[0.98] transition-all text-lg" style={{paddingTop: '1.125rem', paddingBottom: '1.125rem'}}>ไปหน้าชำระเงิน <ChevronRight size={22} /></button>
          ) : (
            <button onClick={submitOrder} disabled={isSubmitting} className={`w-full text-white rounded-3xl py-4.5 font-black shadow-2xl flex justify-center items-center gap-2 transition-all text-lg ${isSubmitting ? 'bg-slate-400 scale-100' : paymentMethod === 'cash' ? 'bg-emerald-600 hover:bg-emerald-700 active:scale-[0.98]' : 'bg-orange-600 hover:bg-orange-700 active:scale-[0.98]'}`} style={{paddingTop: '1.125rem', paddingBottom: '1.125rem'}}>
